...
- 代价：输入 token 更高（计费口径清洗 6.6×、分类 12.7×，见 [../../signalchain/SYSTEM1.md](../../signalchain/SYSTEM1.md) §13.4.1）；短判据已砍 38%，进一步压缩空间有限
...
- **SHA256 逐字节一致**；操作链（字段 × 操作 × 改动数 × 错误数，18 行）完全一致
- 重构 `categorical_system1.py` 之后重跑，仍然一致

**分类链路**（`report.json` + 4 个 `*_type.json` + `report.xlsx`）：

- 6 个产出中 **5 个一致**：`data_A/C/D_type.json`、`report.json` 逐字节相同
- **`data_B_type.json` 有分歧**（200B vs 189B）：系统二把 `id` 也判为分类变量，
  系统一没判。`id` 是标识符，判为分类变量不合理，这一处**系统一更合理**
- 分歧**不传播**：R 脚本为 data_B 选的是 `gender` vs `education`，
  所以 `report.json` 两边完全相同，统计结论不受影响
- `report.xlsx` 同为 11961B，仅字节不同（xlsx 内嵌时间戳）

**可复现性**（各跑两次，清空产物与缓存后）：

| | 第 1 次 | 第 2 次 |
| --- | --- | --- |
| 纯系统二 `data_B_type.json` | 2D3EF21C901210A1 | 2D3EF21C901210A1 |
| 纯系统一 `data_B_type.json` | 31EBC9D7D9CE474F | 31EBC9D7D9CE474F |

两个引擎各自**完全可复现**，所以上表那处差异是真实的判断分歧，不是随机噪声。

结论：**清洗链路逐字节复现；分类链路 5/6 一致，唯一分歧处系统一更合理且不影响统计报告**。
"完整一致"只对清洗链路成立，分类链路应表述为"5/6 一致 + 1 处系统一更合理"。

## Jev 在本项目的结构性劣势

信号码机制本身就是**"压输出"的优化**——LLM 输出贵，所以让它只输出 1 个字符，输出成本压到接近零。

Jev 的核心优势是**输出免费**。但在本项目里，输出本来就已经接近零成本，这个优势没有边际收益；
而 Jev 的输入侧代价全部暴露（criteria 每字段重复一份，清洗链路输入 token 是系统二的 6.6 倍）。

净效果：**Jev 在本项目是结构性劣势，不是甜点区。** Jev 的甜点区是"输出压不下去"的场景。

可复用的判断规则：**接入新引擎前，先看它的强项在当前系统里是不是已经被优化掉了。
如果是，那个优势不会兑现，只剩代价。**

> 甜点区的结构模型（S / K / N / P）见 [../../signalchain/SYSTEM1.md](../../signalchain/SYSTEM1.md) §13.4.6。
