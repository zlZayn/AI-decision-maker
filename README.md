...
你收到很多 CSV 文件，有的把性别列叫 `sex`，有的叫 `gender`，有的叫 `xb`。
程序怎么自动认出这是性别列？哪些列是分类变量？分类变量有没有顺序？

一句话说清，SignalChain 做两件事：

- **认字段** —— AI 决策"这是性别列"，程序执行清洗
- **认分类变量** —— AI 决策"这是有序变量"，程序选择统计方法

设计原则：AI 只传必要信息（字段名 + 类型 + 样本值），不传原始数据；AI 输出强制格式，程序严格校验。
...
- 代码配置变更（路由表、操作注册表、标准列名等）—— 只失效对应引擎的分区
- 字段数量或字段名变更
- 样本值发生显著变化

清除缓存：删除 `signal_cache.json` 文件（一次清掉全部引擎）。
