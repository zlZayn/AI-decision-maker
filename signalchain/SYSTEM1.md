...
> **token 量确实更高**，但同口径下清洗链路是 4.0×（不是原先记的 31×），分类链路 13.3×（§13.4）——
...
但输入 token 明显更高 —— 同口径下清洗链路 4.0×、分类链路 13.3×
...
统一到本地 DeepSeek tokenizer（`signalchain/tokenizer.count_tokens`）后重测。
该 tokenizer 与 DeepSeek 官方配置一致（`LlamaTokenizerFast` / `model_max_length` 16384 / `add_bos_token` false / `legacy` true）；
`tokenizer_config.json` 的 `chat_template` 与官方有差异，但 `count_tokens()` 走直接 encode、不走 chat template，不影响计数。

清洗链路，medical.csv（5 字段）：

| | 系统二 | 系统一 | 倍数 |
| --- | --- | --- | --- |
| 同口径输入 token | **157**（2 次调用）| **633**（1 次请求）| **4.0×** |
| 各自 API 回报的输入 token | 355（2 次）| 2335（1 次，TypeSafe 口径）| 6.6× |

三个文件的同口径合计：系统二 **477** / 系统一 **1861** → **3.9×**。

分类链路，4 个文件（同口径）：

| | 系统二 | 系统一 | 倍数 |
| --- | --- | --- | --- |
| 同口径输入 token | **384** | **5119** | **13.3×** |
| 各自 API 回报的输入 token | 1463 | 18533 | 12.7× |

**结论：31× 不成立。清洗链路同口径 4.0×、各自计费口径 6.6×；分类链路同口径 13.3×。**

**分类链路的 13.3× 是结构性的，单独立条**：
...
同口径下分类链路比清洗链路贵 3 倍多（13.3× vs 4.0×），根因就在这个乘法关系，
...
上表的 4.0× / 13.3× 是 **token 数**之比，不能直接读成成本倍数：两边单价不同，而且币种不同。

| | 输入单价 | 输出单价 | 币种 |
| --- | --- | --- | --- |
| 系统一（Jev）| $0.042 / M（未核实）| $0（不计费）| USD |
| 系统二（LLM）| ¥1.0 / M | ¥2.0 / M | CNY |

两个单价分属不同币种，**本身也不能直接比大小**；要比就需要一个汇率假设。
于是成本倍数只能写成随汇率变化的形式：

```
清洗链路每文件：系统一 2335 × $0.042/M = $0.0000981   ÷  系统二 ¥0.000377  →  0.26 × 汇率
分类链路每文件：系统一 4633 × $0.042/M = $0.0001946   ÷  系统二 ¥0.000400  →  0.49 × 汇率
```

以 1 USD = 7.1 CNY 为例（**该汇率为假设，不是官方数据**）：

| 链路 | token 倍数（同口径）| 成本倍数（该汇率下）| 每文件成本（各自币种）|
| --- | --- | --- | --- |
| 清洗（medical）| 4.0× | **≈ 1.9×** | 系统一 $0.000098 vs 系统二 ¥0.000377 |
| 分类 | 13.3× | **≈ 3.5×** | 系统一 $0.000195 vs 系统二 ¥0.000400 |

**结论：Jev 单次成本高于系统二**（清洗 ≈1.9×、分类 ≈3.5×，按假设汇率）。
两项都低于对应的 token 倍数（4.0× / 13.3×），因为 Jev 的单 token 价格更低、且输出不计费 ——
**但"比 token 倍数低"不等于"更便宜"，绝对成本仍是 Jev 更高。**

换汇率时按上式的系数折算即可（清洗 0.26、分类 0.49），无需重测 token。
...
数一遍并打印（"同口径（DeepSeek tokenizer）请求体 = N tokens"），
与系统二提示词的 token 数来自同一把尺子。
