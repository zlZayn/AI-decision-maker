...
两种 token 比要分清。**成本分解必须用计费口径**——各家用自家 tokenizer 计数、并按其计数计费：

| 链路 | 同口径 token 比 | **计费口径 token 比** | 成本倍数 |
| --- | --- | --- | --- |
| 清洗（medical）| 4.03×（§13.4.1）| **6.58×**（2335 ÷ 355）| ≈ 1.9× |
| 分类（每文件）| 13.33×（§13.4.1）| **12.67×**（4633 ÷ 366）| ≈ 3.5× |

拿成本倍数去比**同口径**的 4.0× / 13.3× 是口径混用：系统一按 TypeSafe 计数（medical 2335）计费，
系统二按自家计数（355）计费，两者相差约 3.7 倍。
